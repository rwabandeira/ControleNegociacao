...
begin
  ...
  if pcRelacao.ActivePage <> tsResultados then begin
    Application.MessageBox('Necessário estar na aba de resultados!', 'Atenção', 0);
    Exit;
  end;

  if sgNegociacoes.Cells[cNegociacao_Id, sgNegociacoes.FixedRows] = '' then begin
    Application.MessageBox('Não há dados para gerar relatório!', 'Atenção', 0);
    Exit;
  end;
