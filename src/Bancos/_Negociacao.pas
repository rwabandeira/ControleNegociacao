...
uses
  SysUtils, _DB, SqlExPr, Forms, _LimiteCredito, _ItemNegociacao;
...
begin
  try
    ...
    if status = 'APR' then
      pesq.SQL.Add('  DATA_APROVACAO = current_date')
    else if status = 'CON' then
      pesq.SQL.Add('  DATA_CONCLUSAO = current_date')
    else if status = 'CAN' then
      pesq.SQL.Add('  DATA_CANCELAMENTO = current_date');

    pesq.SQL.Add('where NEGOCIACAO_ID = :P2');

    pesq.ParamByName('P1').AsString := status;
    pesq.ParamByName('P2').AsInteger := negociacao_id;
    pesq.ExecSQL;
  finally
    Result := True;
  end;
